  ...
  return (
    <div className="space-y-3 pb-12">
      ...
      {reservas.map(reserva => {
        ...
        return (
          <ReservaPixItem key={reserva.id} reserva={reserva} produto={produto} onUpdate={onUpdate} />
        )
      })}
    </div>
  )
...
function ReservaPixItem({
  ...
  return (
    <div className={`border border-[#262626] p-4 transition-all ${confirmado ? 'opacity-50' : ''}`}>
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1">
          <p className="font-archivo font-medium text-[#FAFAFA] text-sm">{reserva.clienteNome}</p>
          ...
          <p className="font-display text-lg font-medium text-[#FAFAFA] mt-1">
        ...
        <button
          ...
          className="shrink-0 flex items-center gap-2 bg-[#FAFAFA] text-[#0A0A0A] text-[10px] font-archivo
            font-medium tracking-widest uppercase px-4 py-2.5 transition-all hover:bg-[#E5E5E5]
            disabled:opacity-50"
        >
          <Check size={12} />
